...
def seeding_scheme_lookup(seeding_scheme: str):
    ...
    # Support explicit context width via patterns like 'lefthash_1', 'lefthash_10'
    if seeding_scheme.startswith("lefthash_"):
        try:
            cw = int(seeding_scheme.split("_", 1)[1])
            assert cw >= 1
        except Exception:
            raise ValueError(f"Invalid lefthash_ pattern for seeding_scheme: {seeding_scheme}")
        prf_type = "additive_prf"
        context_width = cw
        self_salt = False
        hash_key = 15485863
    elif seeding_scheme.startswith("simple_") and seeding_scheme != "simple_1":
        # Backward compat: keep legacy 'simple_1' mapping below; otherwise parse 'simple_<n>'
        try:
            cw = int(seeding_scheme.split("_", 1)[1])
            assert cw >= 1
        except Exception:
            raise ValueError(f"Invalid simple_ pattern for seeding_scheme: {seeding_scheme}")
        prf_type = "additive_prf"
        context_width = cw
        self_salt = False
        hash_key = 15485863
    elif seeding_scheme == "simple_1" or seeding_scheme == "lefthash":
...
def minskipgram_prf(input_ids: torch.LongTensor, salt_key: int, k: int = 2) -> int:
    ...
    # Guard against short contexts where fewer than 2 tokens are available.
    hashed = hashint(salt_key * input_ids)
    # Flatten to ensure we are working with a simple 1D sequence of token hashes.
    hashed_flat = hashed.view(-1)
    if hashed_flat.numel() < 2:
        # Fallback: behave like a simple minhash on the available tokens.
        return hashed_flat.min().item()
    # Build all pairwise combinations and take the min over their products.
    pairs = list(combinations(hashed_flat.tolist(), 2))
    skipgrams = torch.as_tensor(pairs, dtype=torch.long)
