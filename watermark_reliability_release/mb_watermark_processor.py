...
class WatermarkBase:
    ...
    def _seed_rng(self, input_ids: torch.LongTensor) -> None:
        ...
        # Some callers (e.g., quantile watermark processor) pass a tensor of shape
        # [1, L] instead of a flat prefix. For PRF schemes that rely on positional
        # indexing in the context (e.g., anchored_skipgram_prf), we must operate on
        # a 1D sequence of token ids. Reduce any 2D [1, L] input to a flat view.
        if input_ids.dim() == 2:
            input_ids = input_ids.view(-1)
        elif input_ids.dim() != 1:
            raise ValueError(
                f"_seed_rng expects a 1D prefix or a [1, L] tensor; got shape {tuple(input_ids.shape)}"
            )

        # Use as much context as available, up to context_width.
        # Original code required at least context_width tokens and raised an error
        # when the prefix was shorter. For downstream tasks (e.g., seq2seq models
        # with short decoder prefixes), we relax this requirement and fall back to
        # the entire available prefix when it is shorter than context_width.
        ctx_width = min(input_ids.shape[-1], self.context_width)

        prf_key = prf_lookup[self.prf_type](input_ids[-ctx_width:], salt_key=self.hash_key)
...
class WatermarkLogitsProcessor(WatermarkBase, LogitsProcessor):
    ...
    def _calc_greenlist_mask(
        ...
        # Be robust to out-of-range indices by clamping.
        green_tokens_mask = torch.zeros_like(scores, dtype=torch.bool)
        vocab_size = scores.shape[-1]
        for b_idx, greenlist in enumerate(greenlist_token_ids):
            if greenlist is None or len(greenlist) == 0:
                continue
            idx = torch.as_tensor(greenlist, device=scores.device, dtype=torch.long)
            idx = idx[(idx >= 0) & (idx < vocab_size)]
            if idx.numel() == 0:
                continue
            green_tokens_mask[b_idx].scatter_(0, idx, True)
...
class WatermarkDetector(WatermarkBase):
    ...
    * z_threshold -> Changing this threshold will change the sensitivity of the detector.
    ...
    def dummy_detect(
        ...
        score_dict.update(dict(bit_match=None))  # Boolean field, use None instead of NaN
        ...
        if return_z_at_T:
            ...
            # For multi-T bit-accuracy curves we expose an empty tensor here
            # so downstream code sees a consistent schema even when prefix
    ...
    def _score_sequence(
        ...
        **kwargs,
        ...
        if return_z_at_T:
            ...
            # Also expose a prefix-wise bit accuracy curve bit_acc_at_T when we
            # have access to the gold message. We reconstruct prefix statistics
            # by replaying the n-gram scan and incrementally decoding the
            # message from partial position-wise colorlist counts.
            if (
                return_bit_match
                and isinstance(gold_message, str)
                and gold_message != ""
            ):
                bit_acc_seq = []

                M = self.base
                converted_len = self.converted_msg_length
                # Per-position counts of colorlist hits accumulated up to the
                # current prefix length.
                prefix_green_cnt = {
                    pos: [0 for _ in range(M)]
                    for pos in range(1, converted_len + 1)
                }
                prefix_pos_cnt = {
                    pos: 0 for pos in range(1, converted_len + 1)
                }

                increment = self.context_width - self.self_salt
                # Replay the sequential scoring pass token by token
                for idx_tok, idx in enumerate(
                    range(self.context_width, len(input_ids) + self.self_salt)
                ):
                    pos = increment % converted_len + 1
                    ngram = input_ids[idx - self.context_width : idx + 1 - self.self_salt]
                    ngram = tuple(ngram.tolist())
                    target = ngram[-1]
                    prefix_ids = ngram if self.self_salt else ngram[:-1]
                    if self.use_fixed_position:
                        colorlist_flag, _ = self._get_ngram_score_cached(
                            prefix_ids, target
                        )
                    else:
                        colorlist_flag, pos = self._get_ngram_score_cached(
                            prefix_ids, target
                        )
                    for f_idx, flag in enumerate(colorlist_flag):
                        if flag:
                            prefix_green_cnt[pos][f_idx] += 1
                    prefix_pos_cnt[pos] += 1
                    increment += 1

                    # Decode message digits at this prefix length
                    digits_T = []
                    for p in range(1, converted_len + 1):
                        if prefix_pos_cnt[p] <= 0:
                            digits_T.append(0)
                        else:
                            counts_p = prefix_green_cnt[p]
                            # Tie-break by earliest index to keep behavior stable
                            best_digit = max(
                                range(M), key=lambda d: counts_p[d]
                            )
                            digits_T.append(best_digit)

                    try:
                        cb_T, tb_T, _ = self._compute_ber(digits_T, gold_message)
                        bit_acc_T = cb_T / tb_T if tb_T > 0 else float("nan")
                    except Exception:
                        bit_acc_T = float("nan")
                    bit_acc_seq.append(float(bit_acc_T))

                # Anchor the final prefix value to the overall bit_acc so that
                # the curve ends at the scalar metric even when tie-breaking
                # behavior differs slightly from the global decoder.
                final_bit_acc = score_dict.get("bit_acc", None)
                try:
                    final_val = float(final_bit_acc)
                except Exception:
                    final_val = float("nan")
                if bit_acc_seq and np.isfinite(final_val):
                    bit_acc_seq[-1] = final_val
                ...
                # Maintain schema when we cannot compute prefix bit accuracy.
                score_dict.update(
                    dict(bit_acc_at_T=torch.tensor([], dtype=torch.float32))
                )
    ...
    def _predict_message(self, position_cnt, green_cnt_by_position, p_val_per_pos,
        ...
        random_prediction_list = [msg_prediction]
        if not confidence_per_pos:
            return random_prediction_list, random_prediction_list, elapsed_time
