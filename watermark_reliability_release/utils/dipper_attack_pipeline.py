...
# Try to use NLTK sentence tokenizer if available locally; otherwise fall back to a regex splitter.
try:
...
def safe_sent_tokenize(text: str):
    """Sentence tokenize without forcing downloads.

    If NLTK's punkt is available locally, use it. Otherwise fall back to a
    simple regex that splits on [.?!] followed by whitespace.
    """
...
def generate_dipper_paraphrases(
    data,
    model_name="/data1/PLM/kalpeshk2011/dipper-paraphraser-xxl",
    no_ctx=True,
    sent_interval=3,
    start_idx=None,
    end_idx=None,
    ...
    for idx, dd in tqdm.tqdm(enumerate(data), total=len(data)):
        ...
        if "w_wm_output_attacked" not in dd:
            ...
            sentences = safe_sent_tokenize(input_gen)
    ...
    if "sampled_positions" in data.column_names:
        data = data.add_column("w_wm_output_sampled_positions", data["sampled_positions"])

    # For attacked output, set empty sampled_positions since it's not watermarked
    w_wm_output_attacked_sampled_positions = [""] * len(data)
    data = data.add_column("w_wm_output_attacked_sampled_positions", w_wm_output_attacked_sampled_positions)
