...
def infer_length_column(base_col_name, dataframe, args=None):
    ...
    # Some callers (e.g., attack_pipeline) may not define the
    # `ignore_repeated_ngrams` flag on their argparse Namespace. Fall
    # back to the legacy behavior (use `_num_tokens_scored` when
    # available) in that case.
    ignore_repeated_ngrams = False
    if args is not None:
        ignore_repeated_ngrams = bool(getattr(args, "ignore_repeated_ngrams", False))
